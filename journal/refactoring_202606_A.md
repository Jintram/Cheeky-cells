...
- [x] ~~Do an import-only smoke run of each of the five active entry points
  (just up to the first orchestrator call + config construction — no need to
  open the napari GUI).~~
- [x] ~~`grep -r "cheeky_cells.misc\|cheeky_cells.devtools\|cheeky_cells.machine_learning.applying\|plotting_generic\|annotation_aided_OLD" cheeky_cells/ pipelineclean_*.py`
