...
Then, user-defined pipeline goes:
...
** U-net prediction part **

The segmentation loop (above) instantiates a U-net.

- Uses `cheeky_cells/machine_learning/model/unet_model.py` 
    - which contains class UNet that defines the U-net
        - Adheres to Torch practices (and is rather technical/fundamental)
        - Contains types of operations (in `__init__`) and forward pass (in `forward()`)
        - Imports `unet_parts.py` which defines subparts of network interaction.
        
Training and handling/formatting training data is done in earlier phases 
(related code is found in `cheeky_cells/machine_learning/trainer/` and
`cheeky_cells/machine_learning/datasetclass` respectively).